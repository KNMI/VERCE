...
from src.wfs_input_generator.input_file_generator import InputFileGenerator
import json
import csv
import StringIO
...
class MomentTensor(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        if self.path == "components-image":
            ...
            imageloc = self.store.produceImage(id, mrr=float(mrr), mtt=float(mtt), mpp=float(mpp), mrt=float(mrt),
...
class SolverParFile(resource.Resource):
    ...
    def render_POST(self, request):
        ...
        """
        solver_conf = json.loads(str(request.args["jsondata"].pop(0)))
        ...
        par_file = gen.write_par_file(format=self.path.upper())
        return par_file
