...
RUN_ID=$(python verce-hpc-pe/src/PEDataExtractor.py conf root runId)
echo $USER_ID
...
echo globus-url-copy -cd -r -v -c -fast -cc 4 -stall-timeout 30 $SOURCE_BASE_URL/$i gsiftp://dir-irods.epcc.ed.ac.uk/~/verce/$i
globus-url-copy -cd -r -v -c -fast -cc 4 -stall-timeout 30 $SOURCE_BASE_URL/$i gsiftp://dir-irods.epcc.ed.ac.uk/~/verce/$i
done
