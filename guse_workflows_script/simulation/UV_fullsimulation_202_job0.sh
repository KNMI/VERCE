...
#cd bin/
python $RUN_PATH/verce-hpc-pe/src/bulk_run_specfem_movie.py $RUN_PATH/jsonout_inputgen $RUN_PATH/jsonout_decompose $RUN_PATH/solverconf srun_ps $COUNTER 
...
srun_ps python -m dispel4py.new.processor -t mpi  test.hpc.forwardsim.postproc_graph
#python -m dispel4py.new.processor -t multi -n 50  test.hpc.forwardsim.postproc_graph

echo "generation of kmz file..."
python -m dispel4py.new.processor -t simple test.hpc.forwardsim.postproc_graph_kml
