...
Ext.define('CF.controller.Map', {
  ...
  //  stores: ['StationStore'],
    ...
    init: function() {
      ...
      if (this.mapPanel.map.getLayersByName("Stations")!="")
		...
		this.stationLayer.events.on({
                ...
                grid.getView().focusRow( stationStore.getByFeature(feature) ); 
                feature.popup = popup;
                map.addPopup(popup);
},
    ...
    onEventSearch: function(button) {  
		 ...
		 //
		//   this.eventstore.each(function(rec) {this.eventstore.remove(rec)},this)
	      this.eventstore.removeAll();
       ...
       this.eventLayer = new OpenLayers.Layer.Vector("Events", {
                ...
                protocol: new OpenLayers.Protocol.HTTP({
             //   url:'/hpc-usecase-portlet/data/stations.xml',
               url : '/j2ep-1.0/edp/services/event/search?limit=100&format=json&'+form.getValues(true)+"&"+commonsValues,
      ...
      if (this.mapPanel.map.getLayersByName("Events")!="")
		...
		this.eventLayer.events.on({
                ...
                grid.getView().focusRow( eventStore.getByFeature(feature) ); 
                feature.popup = popup;
                map.addPopup(popup);
},
		       'featureunselected': function(evt){
