            ...
            Ext.Ajax.request({
                ...
                success: function(response) {
                    ...
                    velocityCombo.store.addListener('refresh', function() {
                        ...
                        RS.app.getController('Map').getStore('SolverConf').loadData(object.fields);

                        // HACK ensure correct event binding order by binding here
                        var eventLayer = RS.app.getController('Map').mapPanel.map.getLayersByName('Events')[0];
                        RS.app.getController('Map').getStore('Event').bind(eventLayer);
                            ...
                            featuresadded: function(event) {
                                event.features.forEach(function(feature) {
                                    if (object.events.indexOf(feature.data.eventId) >= 0) {
                                        RS.app.getController('Map').mapPanel.map.getControl('dragselect').select(feature);
                                    } else {
                                        RS.app.getController('Map').mapPanel.map.getControl('dragselect').unselect(feature);
                                    }
                                });
                        ...
                        // reuse events
                        RS.app.getController('Map').getEvents(RS.app.getController('Map'), prov_object.quakeml.url.replace(/http:\/\/[^\/]*\//, '/'));
                        ...
                        // HACK ensure correct event binding order by binding here
                        var stationLayer = RS.app.getController('Map').mapPanel.map.getLayersByName('Stations')[0];
                        RS.app.getController('Map').getStore('Station').bind(stationLayer);
                        ...
                        // reuse stations
                        RS.app.getController('Map').getStations(RS.app.getController('Map'), prov_object.stations.url.replace(/http:\/\/[^\/]*\//, '/'), stationFileType);
                    ...
                    RS.app.getController('Map').getStore('SolverConf').addListener('refresh', function() {
