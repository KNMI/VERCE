			...
			var InputMoveBoxes = A.Component.create(
				{
					...
					prototype: {
						...
						_renderButtons: function() {
							...
							// Admin layout Fix. See below for more details
							fixRenderingLayoutIssue(contentBox);
...
//Admin layout Fix. The code fails to find the class names .move-arrow-buttons, .left-selector-column and 
//.right-selector-column because the respective class names have been amalgamated 
//with a generated span tag during HTML rendering.
function fixRenderingLayoutIssue(contentBox)
{
	moveButtonsColumn = contentBox.one('.move-arrow-buttons');
	if(!moveButtonsColumn)
	{
		renderedClassName=contentBox._node.children[0].childNodes[1].className;
		moveButtonsColumn=contentBox.one("."+renderedClassName);							
		spanTag=renderedClassName.split("move-arrow-buttons")[0];
		correctClassNames=spanTag + " move-arrow-buttons";
		moveButtonsColumn._node.className=correctClassNames;
		
	}
	leftColumn = contentBox.one('.left-selector-column');							
	if(!leftColumn)
	{

		renderedClassName=contentBox._node.children[0].childNodes[0].className;
		leftColumn=contentBox.one("."+renderedClassName);							
		spanTag=renderedClassName.split("left-selector-column")[0];
		correctClassNames=spanTag + " left-selector-column";
		leftColumn._node.className=correctClassNames;
		
	}
	rightColumn = contentBox.one('.right-selector-column');							
	if(!rightColumn)
	{

		renderedClassName=contentBox._node.children[0].childNodes[2].className;
		rightColumn=contentBox.one("."+renderedClassName);							
		spanTag=renderedClassName.split("right-selector-column")[0];
		correctClassNames=spanTag + " right-selector-column";
		rightColumn._node.className=correctClassNames;
