...
class StreamProducer(GenericPE):
    ...
    def _process(self, inputs):
        ...
        stationxml = inputs["input"]["stationxml"]
        quakeml = inputs["input"]["quakeml"]
        misfit_type = inputs["input"]["misfit_type"]
        parameters = inputs["input"]["parameters"]

        output_folder = inputs["input"]["output_folder"]
        ...
        for d in data:
            data_st += obspy.read(d)

        synth_st = obspy.Stream()
        for s in synth:
            synth_st += obspy.read(s)
...
class PyflexPE(GenericPE):
    ...
    def _process(self, inputs):
        ...
        ws = WindowSelector(observed=ip["data_trace"],
                            ...
                            config=config,
                            event=obspy.readEvents(ip["quakeml"])[0],
...
class MatchComponents(GenericPE):
    ...
    def _process(self, inputs):
        ...
        return {'output': [station_id, image_type, value, output_folder]}
