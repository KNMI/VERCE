...
def template_kml_placemark(st,filename,lon,lat,z):
    ...
    return kml
def globe_template_kml_placemark(st,filename,lon,lat,z):
    kml = '''<Placemark>
                <name>%s</name>
                <styleUrl>#BStyle</styleUrl>
                <description><![CDATA[Synthetics <h3>Displacement<br> <img class="imageStyle"  src="%s" width="420" /><img class="imageStyle"  src="%s" width="420" /><img class="imageStyle"  src="%s" width="420" />]]></description>
                  <Point>
                    <coordinates>%s,%s,%s</coordinates>
                  </Point>
             </Placemark>
             ''' %(st,filename[0],filename[1],filename[2],lon,lat,z)
    return kml
...
class kmlGenerator_INGV(IterativePE):
    ...
    def _process(self,inputs):
        ...
        for s in stations:
            ...
            if "SOLVER_NAME" in os.environ and os.environ["SOLVER_NAME"]=="SPECFEM3D_GLOBE":
                tmp_kml = tmp_kml + globe_template_kml_placemark(st, filenames, lon, lat, z)
            else:
